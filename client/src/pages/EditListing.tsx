...
const EditListingPage = () => {
	...
	useEffect(() => {
		...
		if (url.includes('/listing/edit/')) {
			const id = url.replace('/listing/edit/', '')
			...
			fetch(`/api/listing/${id}`)
				...
				.then((data) => {
					...
					if (userRef !== currentUser?.id) {
						toast.error('You can edit only your listing')
						navigate('/profile')
					}
	...
	const getImages = () => {
		return form.images.map((image: string | File) => {
			...
			else return window.URL.createObjectURL(image)
		})
	}
	...
	const setImages = (imageToRemove: number) => {
		...
		if (typeof form.images[imageToRemove] === 'string')
			setDeletedImages((prev) => [
				...prev,
				form.images[imageToRemove] as string,
			])

		setForm((prev) => ({
			...prev,
			images: prev.images.filter((_, idx: number) => idx !== imageToRemove),
		}))
	}
		...
		try {
			...
			for (let i = 0; i < form.images.length; i++) {
				if (typeof form.images[i] === 'string')
					images.push(form.images[i] as string)
				else promises.push(uploadImages(form.images[i] as File))
			}

			Promise.all(promises)
				.then((urls) => {
					try {
						fetch(`/api/listing/update/${form.id}`, {
							method: 'PATCH',
							headers: {
								...
								if (!data.success) {
									...
									if (deletedImages.length > 0) {
										...
										for (let i = 0; i < deletedImages.length; i++) {
											deletedImagesPromises.push(deleteImage(deletedImages[i]))
										}
			...
			<section className='max-w-screen-2xl min-h-[var(--container-min-height)] mx-auto py-4 md:py-8 px-4 sm:px-6 lg:px-8 flex flex-col'>
				...
				<form
					...
					<div className='h-fit flex-1 flex flex-col gap-6 items-center sticky left-0 right-0 md:top-6 lg:top-8 '>
						{/* Image Preview */}
						{form.images.length > 0 && (
							<Carousel images={getImages()} setImages={setImages} />
						)}
